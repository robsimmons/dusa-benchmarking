...
function makeEdges(numEdges, type) {
  ...
  // mid-random: we want kinda about V^1.5 = E
  //    so we want V(V-1) * P = V^1.5
  //    so we want about P = 2*sqrt(V)/(V - 1)
  //    and we want V = 2*E^(2/3)
  ...
  switch (type) {
    ...
    case 'mid-random': {
      numNodes = Math.floor(Math.pow(numEdges, 2 / 3));
      probabilityOfEdge = (2 * Math.sqrt(numNodes)) / (numNodes - 1);
      break;
    }
    case 'dense-random':
